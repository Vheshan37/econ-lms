...
export function ExamForm({
  ...
  return (
    <div
      className={`bg-white rounded-2xl shadow-sm border p-6 ${continuingExamId ? "border-[#D4AF37] ring-2 ring-[#D4AF37]/20" : "border-gray-200"}`}
    >
      <div className="flex items-center gap-4 mb-6">
        <div
          className={`h-12 w-12 rounded-xl flex items-center justify-center ${isEditing ? "bg-[#D4AF37]/20" : "bg-[#D4AF37]/10"}`}
        >
          {isEditing ? (
        ...
        {isEditing && (
          ...
          >
            <X className="w-4 h-4" />
            {editingExistingMark ? "Cancel Edit" : "Stop Editing"}
          </Button>
        )}
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
              ...
              {examTitleWarning && (
                <div className="space-y-2">
                  <p className="text-xs flex items-start gap-1 text-red-600 bg-red-50 px-3 py-1.5 rounded-lg border border-red-200">
                    ...
                    <span>{examTitleWarning}</span>
                  </p>
                  {duplicateExamId && (
                    <button
                      onClick={() => onGoToDuplicateExam(duplicateExamId)}
                      className="text-xs flex items-center gap-1 text-[#D4AF37] hover:text-[#B5952F] font-medium ml-1"
                    >
                      <Edit2 className="w-3 h-3" />
                      Click here to continue editing that exam instead
                    </button>
                  )}
                </div>
              )}
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                className={`h-12 rounded-xl bg-white text-gray-900 focus:border-[#D4AF37] focus:ring-[#D4AF37] ${examTitleWarning ? "border-red-300" : "border-gray-300"}`}
              />
            </div>
            <div className="space-y-2 lg:col-span-2">
              ...
              <Input
                ...
                value={examDesc}
                onChange={(e) => onExamDescChange(e.target.value)}
                className="h-12 rounded-xl border-gray-300 bg-white text-gray-900 focus:border-[#D4AF37] focus:ring-[#D4AF37] placeholder:text-gray-400"
              />
            </div>
          </div>
        ...
        <div className="flex justify-end gap-3">
          ...
          >
            {isSubmitting ? (
              ...
              `Add ${entries.length} Mark${entries.length > 1 ? "s" : ""} to "${continuingExamTitle}"`
            ) : (
              `Submit Exam (${entries.length} ${entries.length === 1 ? "Student" : "Students"})`
            )}
          </Button>
        </div>
      )}
      ...
      {entries.length === 0 && !editingExistingMark && (
        <div className="text-center py-12 border-2 border-dashed border-gray-200 rounded-xl">
          ...
          <p className="text-gray-500 font-medium">
            {continuingExamId
              ? `Ready to add marks to "${continuingExamTitle}"`
              : "No marks added yet"}
          </p>
          <p className="text-gray-400 text-sm mt-1">
            Add student marks using the form above
          </p>
        </div>
      )}
    </div>
