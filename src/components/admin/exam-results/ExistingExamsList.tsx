              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <div
                      className={`h-10 w-10 rounded-lg flex items-center justify-center ${continuingExamId === exam.id ? "bg-[#D4AF37]/20" : "bg-[#D4AF37]/10"}`}
                    >
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-3 text-sm text-gray-500">
                        ...
                        <span>
                          {exam.marks.length} student
                          {exam.marks.length !== 1 ? "s" : ""}
                        </span>
                        {exam.desc && (
                          <>
                            <span className="w-1 h-1 rounded-full bg-gray-300" />
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <svg
                      className={`w-5 h-5 text-gray-400 transition-transform ${expandedExam === exam.id ? "rotate-180" : ""}`}
                      fill="none"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                    >
                          ...
                          <table className="w-full">
                            <thead>
                              <tr className="border-b border-gray-200 sticky top-0 bg-white z-10">
                                <th className="text-left py-2 px-4 text-xs font-semibold text-gray-700 uppercase bg-white">
                                  Rank
                                </th>
                                <th className="text-left py-2 px-4 text-xs font-semibold text-gray-700 uppercase bg-white">
                                  Index Number
                                ...
                                .map((mark, index) => {
                                  ...
                                  return (
                                    <tr
                                      key={mark.id}
                                      onDoubleClick={() =>
                                        onDoubleClickMark(exam, mark)
                                      }
                                      className="hover:bg-gray-50 transition-colors cursor-pointer"
                                      title="Double‑click to edit this mark"
                                    >
                                      ...
                                      <td className="py-2 px-4">
                                        <span
                                          className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${grade === "A" ? "bg-green-100 text-green-800" : grade === "F" ? "bg-amber-100 text-amber-800" : "bg-yellow-100 text-yellow-800"}`}
                                        >
                                          {marksValue}%
                                        </span>
                                      </td>
                                      <td className="py-2 px-4">
                                        <span
                                          className={`inline-flex items-center justify-center w-8 h-8 rounded-full text-sm font-bold ${grade === "A" ? "bg-green-100 text-green-700" : grade === "F" ? "bg-amber-100 text-amber-700" : "bg-yellow-100 text-yellow-700"}`}
                                        >
                                          {grade}
                                        </span>
                                      </td>
