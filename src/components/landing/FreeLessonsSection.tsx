...
export function FreeLessonsSection() {
  ...
  const [featuredResources, setFeaturedResources] = useState<{
    featuredVideo: FreeResource | null;
    featuredDoc: FreeResource | null;
    featuredQuiz: FreeResource | null;
  }>({ featuredVideo: null, featuredDoc: null, featuredQuiz: null });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [contentResult, resourcesResult, featuredResult] =
          await Promise.all([
        ...
        if (featuredResult.success && featuredResult.data) {
          setFeaturedResources(featuredResult.data);
        }
      } catch (error) {
  ...
  const resourceTypes = [
    {
      ...
      description:
        "Watch comprehensive introductory lessons covering key Economics topics",
    ...
    },
    {
      ...
      description:
        "Access previous A/L Economics papers with detailed marking schemes",
    ...
    },
    {
      ...
      description:
        "Download structured notes, summaries, and reference materials",
    ...
    >
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="text-center mb-16">
          <span className="inline-block px-4 py-2 bg-[#fdf021]/10 text-[#fdf021] rounded-full text-sm font-semibold mb-6">
            🎁 Free Resources
          </span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
          {resourceTypes.map((resource, idx) => (
            ...
            >
              {/* Icon */}
              <div
                className={`w-14 h-14 rounded-xl bg-black/40 flex items-center justify-center mb-4 group-hover:scale-110 transition-transform duration-300`}
              >
              ...
              <div
                className={`inline-block px-3 py-1 ${resource.iconColor} bg-white/10 rounded-full text-xs font-semibold mb-3`}
              >
                {resource.count}
              </div>
              ...
              <div className="absolute bottom-6 right-6 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                <div
                  className={`w-8 h-8 rounded-full ${resource.iconColor} bg-white/10 flex items-center justify-center`}
                >
                  <svg
                    className="w-4 h-4"
                    fill="none"
        ...
        <div className="grid md:grid-cols-3 gap-6">
          ...
          <div
            onClick={() => {
              if (featuredResources.featuredVideo) {
                incrementFreeResourceView(featuredResources.featuredVideo.id);
                if (featuredResources.featuredVideo.url) {
                  window.open(featuredResources.featuredVideo.url, "_blank");
                }
              }
            }}
            className="cursor-pointer bg-linear-to-br from-gray-900 to-gray-950 rounded-2xl p-6 border border-gray-800 hover:border-gray-700 transition-all duration-300 group block"
          >
            ...
            <h4 className="font-bold text-white mb-2">
              {featuredResources.featuredVideo?.title || "No Featured Video"}
            </h4>
            <p className="text-sm text-gray-400 mb-3">
              {featuredResources.featuredVideo
                ? "Most Popular Lesson"
                : "Check back later"}
            </p>
          ...
          <div
            onClick={() => {
              if (featuredResources.featuredDoc) {
                incrementFreeResourceView(featuredResources.featuredDoc.id);
                if (featuredResources.featuredDoc.url) {
                  window.open(featuredResources.featuredDoc.url, "_blank");
                }
              }
            }}
            className="cursor-pointer bg-linear-to-br from-gray-900 to-gray-950 rounded-2xl p-6 border border-gray-800 hover:border-gray-700 transition-all duration-300 group block"
          >
            ...
            <h4 className="font-bold text-white mb-2">
              {featuredResources.featuredDoc?.title || "No Featured Document"}
            </h4>
            <p className="text-sm text-gray-400 mb-3">
              {featuredResources.featuredDoc?.description || "Check back later"}
            </p>
          ...
          <div
            onClick={() => {
              if (featuredResources.featuredQuiz) {
                incrementFreeResourceView(featuredResources.featuredQuiz.id);
                if (featuredResources.featuredQuiz.url) {
                  window.open(featuredResources.featuredQuiz.url, "_blank");
                }
              }
            }}
            className="cursor-pointer bg-linear-to-br from-gray-900 to-gray-950 rounded-2xl p-6 border border-gray-800 hover:border-gray-700 transition-all duration-300 group block"
          >
            ...
            <h4 className="font-bold text-white mb-2">
              {featuredResources.featuredQuiz?.title || "No Featured Quiz"}
            </h4>
            <p className="text-sm text-gray-400 mb-3">Test your knowledge</p>
        ...
        <div className="mt-16 text-center">
          <Link
            href="/free-resources"
            className="inline-block group relative px-10 py-5 bg-linear-to-r from-yellow-500 to-yellow-600 text-black font-bold text-lg rounded-full overflow-hidden transition-all duration-300 hover:shadow-[0_0_40px_rgba(234,179,8,0.6)] hover:scale-105"
          >
