...
export function FreeLessonsPreview({
  data,
  freeResources,
}: FreeLessonsPreviewProps) {
      ...
      <div className="container mx-auto px-4 relative z-10">
          ...
          {data.categories.map((category, idx) => {
            ...
            return (
              ...
              >
                ...
                <div
                  className={`inline-block px-3 py-1 ${colors.iconColor} bg-white/10 rounded-full text-xs font-semibold mb-3`}
                >
                  {category.count}
                </div>
                ...
                <div className="absolute bottom-6 right-6 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                  <div
                    className={`w-8 h-8 rounded-full ${colors.iconColor} bg-white/10 flex items-center justify-center`}
                  >
                    <svg
                      className="w-4 h-4"
                      fill="none"
        ...
        <div className="grid md:grid-cols-3 gap-6">
          ...
          <div className="bg-gradient-to-br from-gray-900 to-gray-950 rounded-2xl p-6 border border-gray-800 hover:border-gray-700 transition-all duration-300 group">
            ...
            <p className="text-sm text-gray-400 mb-3">
              {data.featuredVideo.duration} • Beginner Level
            </p>
          ...
          <div className="bg-gradient-to-br from-gray-900 to-gray-950 rounded-2xl p-6 border border-gray-800 hover:border-gray-700 transition-all duration-300 group cursor-pointer">
            ...
            <h4 className="font-bold text-white mb-2">
              {data.featuredDoc.title}
            </h4>
            <p className="text-sm text-gray-400 mb-3">
              {data.featuredDoc.subtitle}
            </p>
            ...
            <div className="aspect-video bg-gradient-to-br from-purple-900/20 to-black rounded-xl mb-4 flex items-center justify-center">
              <div className="text-center">
                ...
                <div className="text-2xl font-bold text-white">
                  {data.featuredQuiz.questionCount.split(" ")[0]}
                </div>
                <div className="text-xs text-gray-400">Questions</div>
              </div>
            </div>
            <h4 className="font-bold text-white mb-2">
              {data.featuredQuiz.title}
            </h4>
            <p className="text-sm text-gray-400 mb-3">
              {data.featuredQuiz.duration} • Multiple Choice
            </p>
        ...
        <div className="mt-16 text-center">
          ...
          <p className="text-gray-500 text-sm mt-4">
            {data.ctaNote || "No registration required • Instant access"}
          </p>
        </div>
      </div>
    </section>
