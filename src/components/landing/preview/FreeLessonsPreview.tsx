...
export function FreeLessonsPreview({ data, freeResources }: FreeLessonsPreviewProps) {
    const resourceColors = [
        { bgColor: "from-red-500/10 to-red-600/10", iconColor: "text-red-500", borderColor: "border-red-500/20" },
        { bgColor: "from-blue-500/10 to-blue-600/10", iconColor: "text-blue-500", borderColor: "border-blue-500/20" },
        { bgColor: "from-green-500/10 to-green-600/10", iconColor: "text-green-500", borderColor: "border-green-500/20" },
        { bgColor: "from-purple-500/10 to-purple-600/10", iconColor: "text-purple-500", borderColor: "border-purple-500/20" }
    ];
            ...
            <div className="container mx-auto px-4 relative z-10">
                ...
                <div className="text-center mb-16">
                    ...
                    <h2 className="text-4xl md:text-5xl font-serif font-bold text-white mb-6">
                        {data.title || "Explore a Few Lessons Before You Join"}
                    </h2>
                    <p className="text-gray-400 max-w-3xl mx-auto text-lg">
                        {data.description || "අපි ඔබට සුදුසුද යන්න විශ්වාස නැද්ද? එක්වීමට පෙර අපගේ නොමිලේ සම්පත් උත්සාහ කර අපගේ ගුරු කිරීමේ ගුණාත්මකභාවය අත්විඳින්න."}
                    </p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
                    {data.categories.map((category, idx) => {
                        const Icon = categoryIcons[idx] || BookOpen;
                        const colors = resourceColors[idx] || resourceColors[0];

                        return (
                            <div
                                key={idx}
                                className={`group relative bg-gradient-to-br ${colors.bgColor} backdrop-blur-sm rounded-2xl p-6 border ${colors.borderColor} hover:border-opacity-50 transition-all duration-300 hover:scale-105 hover:shadow-2xl cursor-pointer`}
                            >
                                {/* Icon */}
                                ...
                                <div className={`inline-block px-3 py-1 ${colors.iconColor} bg-white/10 rounded-full text-xs font-semibold mb-3`}>
                                    {category.count}
                                </div>
                                ...
                                <div className="absolute bottom-6 right-6 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                    <div className={`w-8 h-8 rounded-full ${colors.iconColor} bg-white/10 flex items-center justify-center`}>
                ...
                <div className="grid md:grid-cols-3 gap-6">
                    ...
                    <div className="bg-gradient-to-br from-gray-900 to-gray-950 rounded-2xl p-6 border border-gray-800 hover:border-gray-700 transition-all duration-300 group">
                        ...
                        <p className="text-sm text-gray-400 mb-3">{data.featuredVideo.duration} • Beginner Level</p>
                    ...
                    <div className="bg-gradient-to-br from-gray-900 to-gray-950 rounded-2xl p-6 border border-gray-800 hover:border-gray-700 transition-all duration-300 group cursor-pointer">
                        ...
                        <p className="text-sm text-gray-400 mb-3">{data.featuredDoc.subtitle}</p>
                    ...
                    <div className="bg-gradient-to-br from-gray-900 to-gray-950 rounded-2xl p-6 border border-gray-800 hover:border-gray-700 transition-all duration-300 group cursor-pointer">
                        <div className="aspect-video bg-gradient-to-br from-purple-900/20 to-black rounded-xl mb-4 flex items-center justify-center">
                            <div className="text-center">
                                ...
                                <div className="text-2xl font-bold text-white">{data.featuredQuiz.questionCount.split(' ')[0]}</div>
                                <div className="text-xs text-gray-400">Questions</div>
                            </div>
                        </div>
                        <h4 className="font-bold text-white mb-2">{data.featuredQuiz.title}</h4>
                        <p className="text-sm text-gray-400 mb-3">{data.featuredQuiz.duration} • Multiple Choice</p>
                ...
                <div className="mt-16 text-center">
                    <button className="group relative px-10 py-5 bg-gradient-to-r from-yellow-500 to-yellow-600 text-black font-bold text-lg rounded-full overflow-hidden transition-all duration-300 hover:shadow-[0_0_40px_rgba(234,179,8,0.6)] hover:scale-105">
                        <span className="relative z-10 flex items-center gap-2">
                            {data.ctaText || "Browse All Free Resources"}
                    ...
                    <p className="text-gray-500 text-sm mt-4">{data.ctaNote || "No registration required • Instant access"}</p>
                </div>
            </div>
        </section>
    );
