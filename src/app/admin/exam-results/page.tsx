...
import { getExams, createExam, updateExam, deleteExam, checkIndexNumber, checkIndexInExam, checkExamExists } from '@/lib/actions/exam-results';
...
export default function ExamResultsPage() {
  ...
  const [deleteAlert, setDeleteAlert] = useState<{ isOpen: boolean; examId: number | null; examTitle: string }>({ isOpen: false, examId: null, examTitle: '' });
  ...
  const checkDuplicateExam = useCallback(async (title: string, date: string) => {
    if (!title.trim() || !date) { setExamTitleWarning(''); setDuplicateExamId(null); return; }
    setIsCheckingExam(true);
    const result = await checkExamExists(title.trim(), date);
    if (result.success && result.exists && result.examData) {
      setExamTitleWarning(`An exam named "${title}" on ${new Date(date).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })} already exists! Please use the edit icon below to continue adding marks.`);
      setDuplicateExamId(result.examData.id);
    } else { setExamTitleWarning(''); setDuplicateExamId(null); }
    setIsCheckingExam(false);
  }, []);

  const checkDuplicateInEntries = useCallback(
    (index: string) => entries.some(entry => entry.indexNumber === index && entry.id !== editingEntryId),
    [entries, editingEntryId]
  );

  const checkIndexInCurrentExam = useCallback(async (index: string) => {
    if (!index.trim()) { setIndexWarning(''); setIsBlocked(false); return; }
    if (editingExistingMark && index === editingExistingMark.indexNumber) { setIndexWarning(''); setIsBlocked(false); return; }
    if (checkDuplicateInEntries(index)) {
      setIndexWarning(`Student #${index} is already in the list. You cannot add the same student twice.`);
      setIsBlocked(true);
    ...
    if (continuingExamId) {
      ...
      if (examResult.success && examResult.hasMarks) {
        setIndexWarning(`Student #${index} already has ${examResult.existingMarks} marks in this exam.`);
        setIsBlocked(true);
        setIsCheckingIndex(false);
        return;
      }
    ...
    if (result.success && result.exists) {
      setIndexWarning(`Student #${index} is already registered in the system and has marks in other exams.`);
    } else {
      setIndexWarning('');
    }
    setIsBlocked(false);
  ...
  const handleIndexChange = (value: string) => {
    ...
    debounceTimerRef.current = setTimeout(() => checkIndexInCurrentExam(value), 500);
  };
  ...
  const handleAddOrUpdateEntry = () => {
    ...
    if (editingExistingMark) {
      ...
      updateExam(editingExistingMark.examId, {
        ...
        .then(r => {
          if (r.success) {
            setSuccessAlert({ isOpen: true, message: `Mark for student #${indexNumber.trim()} updated to ${mark}!` });
            clearAll();
            fetchExams();
          } else {
            setErrorAlert({ isOpen: true, message: r.error || 'Failed to update mark.' });
          }
        })
        .catch(() => setErrorAlert({ isOpen: true, message: 'An unexpected error occurred.' }))
    ...
    if (checkDuplicateInEntries(indexNumber.trim())) {
      setErrorAlert({ isOpen: true, message: 'This student is already in the list.' });
      return;
    }
    const mark = Number(marks);
    if (isNaN(mark) || mark < 0 || mark > 100) {
  ...
  const handleSubmitExam = async () => {
    if (!continuingExamId) {
      if (!examTitle.trim()) { setErrorAlert({ isOpen: true, message: 'Please enter an exam title.' }); return; }
      if (!examDate) { setErrorAlert({ isOpen: true, message: 'Please select an exam date.' }); return; }
  ...
  const handleDeleteConfirm = async () => {
    ...
    if (result.success) {
      ...
      setErrorAlert({ isOpen: true, message: result.error || 'Failed to delete.' });
    }
    ...
    <div className="space-y-8">
      ...
      <ExamForm
        ...
        onExamTitleChange={(v) => {
          ...
          if (examDate) {
            if (debounceTimerRef.current) clearTimeout(debounceTimerRef.current);
            debounceTimerRef.current = setTimeout(() => checkDuplicateExam(v, examDate), 500);
          }
        }}
        onExamDateChange={(v) => {
          setExamDate(v);
          if (examTitle.trim()) {
            if (debounceTimerRef.current) clearTimeout(debounceTimerRef.current);
            debounceTimerRef.current = setTimeout(() => checkDuplicateExam(examTitle, v), 500);
          }
        }}
