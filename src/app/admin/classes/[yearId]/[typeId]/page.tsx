...
function TopicsPageClient({
  ...
  const [classTypeData, setClassTypeData] = useState<ClassTypeData | null>(
    null,
  );
  ...
  const fetchClassTypeData = async () => {
    ...
    if (result.success && result.data) {
      ...
      setErrorAlert({
        isOpen: true,
        message: result.error || "Failed to fetch class type data",
      });
    }
    setIsLoading(false);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      ...
      setErrorAlert({
        isOpen: true,
        message: result.error || "Failed to save topic",
      });
    }
    ...
    <div className="space-y-8 w-full max-w-full overflow-hidden">
      <div className="mx-auto space-y-8 w-full max-w-full">
          ...
          <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4 sm:gap-6">
            <div className="flex items-center gap-4 sm:gap-6">
              ...
              <div>
                ...
                <h1 className="text-xl sm:text-4xl font-bold text-white">Topics</h1>
          ...
          {classTypeData.topics.length === 0 ? (
            <div className="text-center py-16 bg-gray-50 rounded-3xl border-2 border-dashed border-gray-200">
              ...
              <p className="text-gray-400 text-sm mt-2">
                Create your first topic to start adding resources
              </p>
            </div>
          ) : (
            <AnimatePresence>
              {classTypeData.topics.map((topic, index) => (
                <motion.div
                  ...
                  className="group bg-white border border-gray-200 rounded-2xl p-4 sm:p-6 hover:shadow-lg hover:border-[#D4AF37]/30 transition-all cursor-pointer relative overflow-hidden"
                  onClick={() =>
                    router.push(
                      `/admin/classes/${yearId}/${typeId}/${topic.id}`,
                    )
                  }
                >
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      <h3
                        className={`text-lg sm:text-xl font-bold mb-1 transition-colors ${topic.isActive ? "text-gray-900 group-hover:text-[#D4AF37]" : "text-gray-400"}`}
                      >
                        {topic.title}
                        {!topic.isActive && (
                      ...
                      <div className="flex items-center justify-between p-4 bg-white/5 rounded-xl border border-white/10">
                        <div className="space-y-0.5">
                          ...
                          <p className="text-xs text-gray-500">
                            Enable or disable this topic for students
                          </p>
                        </div>
                        <Switch
                          checked={isActive}
        ...
        <AlertDialog
          ...
          onClose={() =>
            setDeleteAlert({ isOpen: false, topicId: null, topicTitle: "" })
          }
