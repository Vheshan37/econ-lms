...
export default function YearsPage() {
  ...
  const handleToggle = async (id: string, currentStatus: boolean) => {
    ...
    if (result.success && result.data) {
      ...
      setErrorAlert({
        isOpen: true,
        message: result.error || "Failed to toggle year status",
      });
    }
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    if (result.success) {
      ...
      setErrorAlert({
        isOpen: true,
        message: result.error || "Failed to delete year",
      });
    }
  };
        ...
        <AnimatePresence>
          {years.map((yr) => (
            <motion.div
              ...
              className={`relative group cursor-pointer overflow-hidden rounded-3xl transition-all ${
                yr.isActive
              ...
              onClick={() =>
                yr.isActive && router.push(`/admin/classes/${yr.id}`)
              }
            >
              {/* Decorative Elements */}
              ...
              <div className="relative z-10 p-5 sm:p-8">
                ...
                {yr.description && (
                  <p
                    className={`text-sm mb-6 line-clamp-2 ${yr.isActive ? "text-gray-400" : "text-gray-600"}`}
                  >
                    {yr.description}
                  </p>
                )}
                ...
                <div
                  className={`flex items-center gap-4 pt-4 border-t ${
                    yr.isActive ? "border-white/10" : "border-gray-300"
                  }`}
                >
                  <div className="flex items-center gap-2">
                    <Layers
                      className={`w-4 h-4 ${yr.isActive ? "text-[#D4AF37]" : "text-gray-500"}`}
                    />
                    <span
                      className={`text-sm ${yr.isActive ? "text-gray-300" : "text-gray-600"}`}
                    >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <span
                      className={`text-xs ${yr.isActive ? "text-gray-400" : "text-gray-500"}`}
                    >
                ...
                <div className="flex items-center gap-4 mb-8">
                  ...
                  <div>
                    <h2 className="text-2xl font-bold text-white">
                      {editingId ? "Edit Year" : "New Academic Year"}
                    </h2>
