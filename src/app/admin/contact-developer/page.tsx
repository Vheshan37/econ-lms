    ...
    try {
      ...
      if (result.success) {
        setStatus({
          type: "success",
          message:
            "Your request has been sent successfully! The developer will review it soon.",
        });
        ...
        >
          ...
          <div className="bg-linear-to-br from-[#D4AF37] to-[#B5952F] rounded-3xl p-6 text-[#1a1a1a]">
            ...
            <p className="text-sm opacity-80 leading-relaxed">
              This form sends your message directly to the developer's inbox for
              faster review and implementation.
            </p>
          </div>
        </motion.div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {requestTypes.map((type) => (
                  <button
                    ...
                    className={`transition-all duration-200 p-4 rounded-2xl border flex items-center gap-4 text-left group ${
                  ...
                  >
                    <div
                      className={`p-2 rounded-lg bg-gray-50 transition-colors ${
                        formData.requestType === type.id ? "bg-white/10" : ""
                      }`}
                    >
                      <type.icon
                        className={`w-5 h-5 ${
