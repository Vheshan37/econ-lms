...
export default function OLResourcesPage() {
  ...
  const [ctaMessage, setCtaMessage] = useState(
    "Want more advanced content? Join our {year} Batch",
  );
  const [heroBadge, setHeroBadge] = useState("Calling all O/L Students");
  const [heroTitle, setHeroTitle] = useState(
    "Start Your A/L Econ Journey Today",
  );
  const [categoryCards, setCategoryCards] = useState([
    {
      title: "Video Lessons",
      description: "Watch expert video tutorials",
      icon: "Play",
      link: "#videos",
    },
    {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (subjectsResult.success && subjectsResult.data) {
        ...
        setErrorAlert({
          isOpen: true,
          message: subjectsResult.error || "Failed to fetch subjects",
        });
      }
      ...
      if (settingsResult.success && settingsResult.data) {
        ...
        if (
          ...
          if (
            data.categoryCards.length === 3 &&
            !data.categoryCards.find((c: any) => c.title === "Video Lessons")
          ) {
            const videoCard = {
              title: "Video Lessons",
              description: "Watch expert video tutorials",
  ...
  const fetchResources = async () => {
    ...
    setIsLoading(true);
    try {
      const resourcesResult = await getFreeResources(
        "Ordinary Level",
        selectedSubject,
      );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        setSuccessAlert({
          isOpen: true,
          message: `Resource ${editingResource ? "updated" : "created"} successfully`,
        });
        handleCloseModal();
        fetchResources();
      } else {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      if (result.success) {
        setSuccessAlert({
          isOpen: true,
          message: `${deleteAlert.type === "subject" ? "Subject" : "Resource"} deleted successfully`,
        });
      } else {
        setErrorAlert({
          isOpen: true,
          message: result.error || `Failed to delete ${deleteAlert.type}`,
        });
      }
    } catch (error) {
      ...
      setErrorAlert({
        isOpen: true,
        message: `An error occurred while deleting the ${deleteAlert.type}`,
      });
    } finally {
      setDeleteAlert({
        isOpen: false,
  ...
  return (
    <div className="space-y-8 w-full max-w-full overflow-hidden">
      ...
      <div className="relative overflow-hidden rounded-3xl bg-linear-to-br from-[#1a1a1a] via-[#2a2a2a] to-[#1a1a1a] p-4 sm:p-8 shadow-2xl">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4 sm:gap-6">
          <div className="flex items-center gap-4 sm:gap-6">
            <div className="h-14 w-14 sm:h-16 sm:w-16 rounded-xl sm:rounded-2xl bg-linear-to-br from-[#D4AF37] to-[#B5952F] flex items-center justify-center shadow-lg flex-shrink-0">
              <BookOpen className="w-7 h-7 sm:w-8 sm:h-8 text-[#1a1a1a]" />
            </div>
            <div>
              ...
              <p className="text-gray-400 text-xs sm:text-sm">
                Manage Ordinary Level free resources for students
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-gray-500">
              Manage the CTA and category cards for the O/L landing page
            </p>
          </div>
          <Button
        ...
        {/* CTA Settings */}
        <div className="space-y-4 mb-8">
          <h3 className="text-lg font-semibold text-gray-800 border-t pt-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2 md:col-span-2">
              <Label htmlFor="ctaMessage">
                CTA Message (use {"{year}"} for dynamic year)
              </Label>
              <Textarea
                id="ctaMessage"
                value={ctaMessage}
      ...
      {selectedSubject ? (
        ...
        <div className="text-center py-20 bg-white/5 rounded-3xl border border-white/10">
          ...
          <p className="text-gray-400">
            Select a subject above to view and manage its resources
          </p>
        </div>
      )}
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {TABS.map((tab) => {
                      ...
                      return (
                        ...
                        >
                          <Icon
                            className={`w-6 h-6 ${resourceType === tab.id ? tab.color : "text-gray-400"}`}
                          />
                          <span
                            className={`text-sm font-medium ${resourceType === tab.id ? "text-gray-900" : "text-gray-600"}`}
                          >
                            {tab.label}
                          </span>
                        </button>
