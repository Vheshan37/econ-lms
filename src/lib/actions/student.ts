    ...
    try {
        ...
        if (filters?.yearId) {
            filteredStudents = students.filter((student: any) =>
                student.classAssignments.some((assignment: any) =>
                    assignment.classType.yearId === filters.yearId
                )
            );
        }
...
export async function assignClassTypes(studentId: string, classTypeIds: string[]) {
    try {
