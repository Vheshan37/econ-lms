...
export async function createResource(data: {
  ...
  try {
    ...
    if (data.topicId) {
      ...
      if (topic) {
        revalidatePath(
          `/admin/classes/${topic.classType.yearId}/${topic.classType.id}/${data.topicId}`,
        );
      }
    } else {
...
export async function updateResource(
  ...
  try {
    ...
    if (topic) {
      revalidatePath(
        `/admin/classes/${topic.classType.yearId}/${topic.classType.id}/${resource.topicId}`,
      );
    }
...
export async function deleteResource(id: string) {
  try {
    ...
    if (resource.topicId && resource.topic) {
      revalidatePath(
        `/admin/classes/${resource.topic.classType.yearId}/${resource.topic.classType.id}/${resource.topicId}`,
      );
    } else {
      revalidatePath("/admin/resources");
    }
